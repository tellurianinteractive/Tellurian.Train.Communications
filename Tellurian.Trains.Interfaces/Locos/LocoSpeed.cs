...
[DataContract]
public readonly struct  LocoSpeed : IEquatable<LocoSpeed>
{
    public static LocoSpeed Set126(byte step) => new (LocoSpeedSteps.Steps126, step);
    public static LocoSpeed Set(LocoSpeedSteps steps, byte step) => new (steps, step);

    private LocoSpeed(LocoSpeedSteps maxStep, byte currentStep)
    {
    ...
    public bool Equals(LocoSpeed other) => other._currentStep == _currentStep && other._maxSteps == _maxSteps;
...
{
    public static LocoSpeed ChangeTo(this LocoSpeed me, float percent) =>
        LocoSpeed.Set(me.MaxSteps, (byte)Math.Round((byte)me.MaxSteps * percent.BetweenZeroAndOne()));
